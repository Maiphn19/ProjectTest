...
//    HttpSession session = createSession();
    ...
    public Account(int acId, String acname, String pass, String per, String fname, String lname, String email) {
        this.acId = acId;
        this.acname = acname;
        this.pass = pass;
        this.per = per;
    ...
    public Account checkAccount(String username, String password) {
        try {
            String strSelect = "select * from Account where [AccountName] = ? and Pass= HASHBYTES('SHA2_256',?)";
    ...
    public void add(String fnamee, String lnamee, String emaill, String per, String acnamee, String passs) {
        try {
            String strAdd = "insert into Account (AccountName,Pass,Permission,Firtname,Lastname,email) values (?,?,?,?,?,?)";
    ...
    public void changePass(String passs, String acnamee) {
        try {
            String strDelete = "update Account set pass= ? where [AccountName] = ?";
    ...
    public Account checkPermisson(String username, String password, String per) {
        try {
            String strSelect = "select * from Account where [AccountName] = ? and Pass= ? and Permission= ?";
            pstm = cnn.prepareStatement(strSelect);
            pstm.setString(1, username);
            pstm.setString(2, password);
            pstm.setString(3, per);
